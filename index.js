...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.gxy6ckd.mongodb.net/?appName=Cluster0`;
...
async function run() {
    try {
        ...
        app.post('/user', async (req, res) => {
            const user = req.body;
            //insert if not exist user
            const query = { email: user.email }
            const existingUser = await userCollection.findOne(query)
            if (existingUser) {
                return res.send({ message: 'user already  here', insertedId: null })
            }
